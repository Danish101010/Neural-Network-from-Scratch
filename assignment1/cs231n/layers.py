...
def svm_loss(x, y):
    ...
    correct_class_scores = x[np.arange(num_examples), y]
    ...
    X_mask[np.arange(num_examples), y] = -np.sum(X_mask, axis=1)
    dx = np.dot(x.T, X_mask)
    dx /= num_examples
...
def softmax_loss(x, y):
    ...
    num_examples = x.shape[0]
    loss = -np.sum(np.log(probs[np.arange(num_examples), y])) / num_examples
    dx = probs.copy()
    dx[np.arange(num_examples), y] -= 1
    dx /= num_examples
